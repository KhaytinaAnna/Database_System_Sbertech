...
StarcounterDB - объектно-ориентироанная база данных, позволяет приложению и базе данных находиться в одном и том же пространстве виртуальной памяти. Поддерживает платформы Linux, Ubuntu, Windows. .NET Core 3.0.100 , SDK для разработки, среда выполнения для производства. К базе данных можно обращаться с помощью SQL-запросов. Серверный код написан с помощью C#, поэтому один из способов работы с Starcounter DB - это использование C# API. Можно работать в редакторе SQL Editor или веб-приложении Starcounter Data Studio. 
...
Тоже создадим свою небольшую базу данных и выполним какие-то запросы:
```using System;
...
public abstract class Person
{
    ...
    public IEnumerable<Person> Elder
    {
        get
        {
            var db = DbProxy.GetContext(this);
            return db.Sql<Person>("SELECT p FROM Person p WHERE p.Age >= 60 ", this);
        }
    }
...
## Какие методы защиты поддерживаются этой СУБД?
Аутентификация и авторизация пользователей с помощью стандартных средств, таких как LDAP и Active Directory. Например, чтобы предотвратить несанкционированный доступ к моделям представления, их можно аннотировать их с помощью [Authorize]:
```[Authorize]
public partial class DogsViewModel: Json
```
Это дает такие эффекты:
 1) Промежуточное ПО предотвращает загрузку этой модели представления, если текущий пользователь не вошел в систему.
 2) Обработчик ввода создается для каждого свойства этой модели представления, проверяя, вошел ли пользователь в систему. Существующие обработчики ввода завернуты в эти средства проверки. Что произойдет, если пользователь не вошел в систему, можно настроить.

Таким образом, только пользователи, вошедшие в систему, могут взаимодействовать с этой моделью представления.
